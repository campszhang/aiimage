...
export async function PATCH(req: NextRequest, { params }: Params) {
  try {
    ...
    if (typeof body.constraints_text === "string")
      push("constraints_text", body.constraints_text.trim());
    ...
    const tx = db.transaction(() => {
      if (makeDefault === true) {
        db.prepare(
          `UPDATE realism_presets SET is_default = 0 WHERE id <> ?`,
        ).run(id);
      ...
      if (updates.length > 0) {
        db.prepare(
          `UPDATE realism_presets SET ${updates.join(", ")} WHERE id = ?`,
        ).run(...values, id);
      }
    });
    tx();

    const row = db.prepare(`SELECT * FROM realism_presets WHERE id = ?`).get(id);
    if (!row) {
...
export async function DELETE(_req: NextRequest, { params }: Params) {
  try {
    ...
    const db = getDb();
    const info = db.prepare(`DELETE FROM realism_presets WHERE id = ?`).run(id);
    if (info.changes === 0) {
      return NextResponse.json({ error: "预设不存在" }, { status: 404 });
    }
